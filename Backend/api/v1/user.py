...
async def findUser(email : str, db: Session = Depends(get_db)) :
...
async def login(user_id : str, db: Session = Depends(get_db)) :
...
async def register(user : UserModelBasic, db: Session = Depends(get_db)) :
    try :
