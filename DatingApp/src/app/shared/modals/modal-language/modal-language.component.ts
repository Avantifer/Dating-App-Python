...
export class ModalLanguageComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    let languageSelected = document.querySelector(
      '.' + this.langService.actualLanguage
    );
  ...
  changeLange(lang: string) {
    ...
    let elementsSelected: NodeListOf<Element> =
  ...
  resetSelectedItems(languageSelected: Element | null) {
    ...
    languageSelected?.classList.add('selected');
    let radioButton: HTMLElement = languageSelected?.childNodes[2].childNodes[0]
